...
After you confirm the projects, the launcher checks all selected projects before asking you to resolve missing Godot editors. The **Editors required** step groups projects that need the same exact editor version and flavour, so one choice can cover several projects. Choose whether to download each available editor, use the shared compatible fallback when one exists, or add the affected projects without an editor.
...
If an editor download fails, later queued downloads continue. The affected project remains marked as missing its editor. Use **Install required editor** on its card when available, or install the editor from **Installs**.
