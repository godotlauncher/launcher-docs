...
If VS Code config files cannot be safely read, Godot Launcher preserves timestamped `.bad` copies and recreates the files it needs. See [Visual Studio Code Setup for Godot](/integrations/vscode-setup-for-godot#recover-from-invalid-vs-code-config-files) for details.
...
[See Install Git for more info](/integrations/install-git)
:::
...
- [Visual Studio Code Setup for Godot](/integrations/vscode-setup-for-godot)
