---
id: launcher-settings
title: Godot Launcher Settings
slug: /settings
description: "Learn how to configure the Godot Launcher using the available settings tabs, including project locations, editor installs, appearance, behavior, tools, and update preferences."
tags:
  - getting-started
  - launcher-settings
---

import ThemedImage from '@theme/ThemedImage';

# Godot Launcher Settings {#launcher-settings}

The **Settings** section lets you tailor Godot Launcher to your workflow. Use it to choose where new projects and editor installs are stored, adjust appearance and startup behavior, configure tool detection, and manage launcher updates.

This page is the settings overview. Each section links to a focused guide with the details for that settings area.

---

## Projects Tab

<ThemedImage
  className="docs-media-frame"
  alt="Settings - Projects Tab"
  sources={{
    light: '/img/docs/1.10/screenshots/screen_settings_projects_light.webp',
    dark: '/img/docs/1.10/screenshots/screen_settings_projects_dark.webp',
  }}
/>

The **Projects** tab controls the default folder used for new projects created through Godot Launcher. Existing projects are not moved when this path changes.

[Project Location Settings](./project-location.mdx)

---

## Installs Tab

<ThemedImage
  className="docs-media-frame"
  alt="Settings - Installs Tab"
  sources={{
    light: '/img/docs/1.10/screenshots/screen_settings_installs_light.webp',
    dark: '/img/docs/1.10/screenshots/screen_settings_installs_dark.webp',
  }}
/>

The **Installs** tab controls where downloaded Godot editor versions are stored and lets you refresh cached release metadata.

[Editor Install Location Settings](./editor-installs-location.mdx)

---

## Appearance Tab

<ThemedImage
  className="docs-media-frame"
  alt="Settings - Appearance Tab Showing the Language Picker"
  sources={{
    light: '/img/docs/1.10/screenshots/screen_settings_appearance_light.webp',
    dark: '/img/docs/1.10/screenshots/screen_settings_appearance_dark.webp',
  }}
/>

The **Appearance** tab controls the launcher theme and interface language. These settings affect Godot Launcher only; they do not change the language of the Godot editor.

[Appearance and Language Settings](./appearance-language.mdx)

---

## Behavior Tab

<ThemedImage
  className="docs-media-frame"
  alt="Settings - Behavior Tab"
  sources={{
    light: '/img/docs/1.10/screenshots/screen_settings_behavior_light.webp',
    dark: '/img/docs/1.10/screenshots/screen_settings_behavior_dark.webp',
  }}
/>

The **Behavior** tab controls confirmation prompts, what happens after launching a project, startup behavior, and optional Windows editor symlink behavior.

[Behavior Settings](./behavior.mdx)

---

## Tools Tab

<ThemedImage
  className="docs-media-frame"
  alt="Settings - Tools Tab"
  sources={{
    light: '/img/docs/1.10/screenshots/screen_settings_tools_light.webp',
    dark: '/img/docs/1.10/screenshots/screen_settings_tools_dark.webp',
  }}
/>

The **Tools** tab shows whether Git and Visual Studio Code are available. Godot Launcher uses these checks to enable Git initialization and VS Code project integration.

[Tool Detection Settings](./tools.mdx)

---

## Updates Tab

<ThemedImage
  className="docs-media-frame"
  alt="Settings - Updates Tab"
  sources={{
    light: '/img/docs/1.10/screenshots/screen_settings_updates_light.webp',
    dark: '/img/docs/1.10/screenshots/screen_settings_updates_dark.webp',
  }}
/>

The **Updates** tab controls background update checks, prerelease updates, manual checks, skipped versions, and update installation prompts.

[Manage Godot Launcher Updates](../updates/manage-launcher-updates.mdx)

---

## Summary

The Settings section helps you:

- Customize default paths for projects and editors.
- Adjust theme, language, and startup behavior.
- Integrate external tools like Git and VS Code.
- Stay up to date with new launcher releases.
